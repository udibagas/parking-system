				...
				<el-table height="calc(100vh - 360px)" :data="files">
					...
					<el-table-column fixed="right" width="80px" align="right">
						<template slot-scope="scope">
							<a
								href="#"
								@click.prevent="restoreDatabase(scope.row)"
								title="Restore Database"
								><i class="el-icon-refresh"></i
							></a>
							<a
								href="#"
								@click.prevent="downloadFile(scope.row)"
								title="Download Database"
								><i class="el-icon-download"></i
							></a>
							<a href="#" @click.prevent="deleteFile(scope.row)" title="Hapus"
				...
				<el-table height="calc(100vh - 360px)" :data="images">
					...
					<el-table-column fixed="right" width="80px" align="right">
						<template slot-scope="scope">
							<a
								href="#"
								@click.prevent="restoreSnapshot(scope.row)"
...
export default {
	filters: {
		asDateTime(v) {
			return this.$moment(v, 'X').format('DD-MMM-YYYY HH:mm:ss')
		},
	},
	data() {
		return {
	...
	methods: {
		...
		downloadFile(file) {
			window.open(
				'/backup?download=' + file.name + '&token=' + this.$store.state.token
			)
		},
		deleteFile(file) {
			this.$confirm('Anda yakin akan menghapus file ini?', 'Peringatan', {
				type: 'warning',
			})
				.then(() => {
					axios
		...
		restoreSnapshot(file) {
			this.$confirm(
				'Anda yakin akan merestore file snapshot ini?',
				'Peringatan',
				{ type: 'warning' }
			)
				.then(() => {
		...
		restoreDatabase(file) {
			this.$confirm(
				'Anda yakin akan merestore file database ini?',
				'Peringatan',
				{ type: 'warning' }
			)
				.then(() => {
