<template>
  <div>
    <div class="text-right">
      <el-button type="primary" icon="el-icon-plus" @click="openForm()" size="small">TAMBAH GATE MASUK</el-button>
    </div>

    <br />

    <el-table :data="tableData.data" height="calc(100vh - 300px)" v-loading="loading">
      <el-table-column prop="shortcut_key" label="#" min-width="30px" align="center"
        header-align="center"></el-table-column>
      <el-table-column min-width="100" label="Status" prop="status">
        <template slot-scope="scope">
          <el-tag effect="dark" :type="scope.row.status ? 'success' : 'info'"
            size="small">{{ scope.row.status ? 'Aktif' : 'Tidak Aktif' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="nama" label="Nama" min-width="100px">
      </el-table-column>
      <el-table-column prop="jenis_kendaraan" label="Jenis Kendaraan" min-width="130px"
      ...
      <el-table-column min-width="150" label="Printer" prop="printer.nama"></el-table-column>
    ...
    <el-dialog v-loading="loading" title="GATE MASUK" :close-on-click-modal="false" :visible.sync="showForm">
      <el-form label-position="left" label-width="150px">
        ...
        <el-form-item label="Jenis Kendaraan" :class="formErrors.jenis_kendaraan ? 'is-error' : ''">
          <el-input placeholder="Jenis Kendaraan" v-model="formModel.jenis_kendaraan"></el-input>
          <div class="el-form-item__error" v-if="formErrors.jenis_kendaraan">
            {{ formErrors.jenis_kendaraan[0] }}
          </div>
        </el-form-item>

        <el-form-item label="Alamat IP Kontroler" :class="formErrors.controller_ip_address ? 'is-error' : ''">
          <el-input placeholder="Alamat IP Kontroler" v-model="formModel.controller_ip_address"></el-input>
          <div class="el-form-item__error" v-if="formErrors.controller_ip_address">
        ...
        <el-form-item label="Printer" :class="formErrors.printer_id ? 'is-error' : ''">
          <el-select v-model="formModel.printer_id" placeholder="Printer" style="width: 100%">
            <el-option v-for="printer in printerList" :value="printer.id" :label="printer.nama"
              :key="printer.id"></el-option>
          </el-select>
        ...
        <el-form-item label="Kamera" :class="formErrors.kamera ? 'is-error' : ''">
          <el-select v-model="formModel.kamera" placeholder="Kamera" style="width: 100%" multiple>
            <el-option v-for="kamera in kameraList" :value="kamera.id" :label="kamera.nama" :key="kamera.id"></el-option>
          </el-select>
