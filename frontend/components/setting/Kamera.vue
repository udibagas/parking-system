<template>
  <div>
    <div class="text-right">
      <el-button type="primary" icon="el-icon-plus" @click="openForm()" size="small">TAMBAH KAMERA</el-button>
    </div>
    ...
    <el-table :data="tableData.data" stripe height="calc(100vh - 300px)">
      ...
      <el-table-column width="120" label="Password" prop="password"></el-table-column>

      <el-table-column width="100" label="Auth Type" prop="auth_type"></el-table-column>

      <el-table-column show-overflow-tooltip min-width="150" label="Snapshot URL" prop="snapshot_url"></el-table-column>

      <el-table-column width="100" label="Status" prop="status" align="center" header-align="center">
        <template slot-scope="scope">
          <el-tag effect="dark" :type="scope.row.status ? 'success' : 'info'" size="small"
            style="width: 100%">{{ scope.row.status ? 'Aktif' : 'Tidak Aktif' }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column fixed="right" width="60px" align="center" header-align="center">
        ...
        <template slot-scope="scope">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-camera" @click.native.prevent="testKamera(scope.row.id)">
                Test Kamera
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-edit" @click.native.prevent="openForm(scope.row)">
                Edit
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete" @click.native.prevent="deleteData(scope.row.id)">
                Hapus
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
    ...
    <el-dialog v-loading="loading" title="KAMERA" :close-on-click-modal="false" :visible.sync="showForm">
      <el-form label-position="left" label-width="150px">
        ...
        <el-form-item label="Username" :class="formErrors.username ? 'is-error' : ''">
        ...
        <el-form-item label="Password" :class="formErrors.password ? 'is-error' : ''">
          <el-input placeholder="Password" v-model="formModel.password"></el-input>
          <div class="el-form-item__error" v-if="formErrors.password">
            {{ formErrors.password[0] }}
          </div>
        </el-form-item>

        <el-form-item label="Otentifikasi" :class="formErrors.auth_type ? 'is-error' : ''">
          <el-select v-model="formModel.auth_type" placeholder="Otentifikasi" style="width: 100%">
        ...
        <el-form-item label="URL Snapshot" :class="formErrors.snapshot_url ? 'is-error' : ''">
          <el-input placeholder="URL Snapshot" v-model="formModel.snapshot_url"></el-input>
        ...
        <el-form-item label="Status" :class="formErrors.status ? 'is-error' : ''">
          <el-select v-model="formModel.status" placeholder="Status" style="width: 100%">
            <el-option v-for="(t, i) in ['Tidak Aktif', 'Aktif']" :value="i" :label="t" :key="i"></el-option>
          </el-select>
